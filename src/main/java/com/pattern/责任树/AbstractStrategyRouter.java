 ...
 * 同时继承 {@code AbstractStrategyRouter} 和实现 {@code StrategyHandler} 接口就可以了。
 * <pre>
 *                              *             | Root |   ----------- 第 1 层策略入口
 *                              *            /       \  ------------- 根据入参 P1 进行策略分发
 *                              *           /         \       +------+      +------+
 ...
 * </pre>
 *
 *  继承 AbstractStrategyRouter<T, R> 抽象类只需要实现abstract方法registerStrategyMapper()即可，
 *  在该方法中实现其不同子节点的路由逻辑。如果子节点路由逻辑比较简单，可以直接通过 if-else 进行分发。
 *  当然如果为了更好地性能、适应更复杂的分发逻辑也可以使用 Map 等保存映射。对于实现了该抽象类的 Router 节点，
 *  只需要调用其 public R applyStrategy(T param) 方法即可获取该节点的期望输出。框架会自动根据定义的路由逻辑将 param 传递到对应的子节点，
 *  再由子节点不断向下分发直到叶子节点或可以给出业务输出的一层。这个过程有点类似递归或者分治的思想。
 *
 *  @see StrategyHandler
 */
@Component
